...
function takeUserData(callback) {
    ...
    var api = "/PublishGraduationCapstone/api/sensitiveword/showSensitiveWord";
    xhttp.open("GET", api);
    xhttp.send();
    var table = new Array;
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            for (var i = 0; i < jsonData.length; i++) {
                var obj = {
                    'wordId': jsonData[i].wordId,
                    'bannedWord': jsonData[i].bannedWord
                };
                table.push(obj);
            };
            console.log(table);
...
takeUserData(function (table) {
    for (var i = 0; i < table.length; i++) {
        var obj = {
            'wordId': table[i].wordId,
            'bannedWord': table[i].bannedWord
        };
        userData.push(obj);
    };
    console.log("lala 1");
    ...
    searchInput.onchange = function (e) {
        ...
        if (searchInputText.isEmpty) {
            ...
            for (var i = 0; i < userData.length; i++) {
                if (userData[i].bannedWord.indexOf(searchInputText) !== -1) {
                    var obj2 = {
                        'wordId': userData[i].wordId,
                        'bannedWord': userData[i].bannedWord
                    };
                    subTable.push(obj2);
                }
            }
        ...
        for (var i = 1 in myList) {
            ...
            var row = `
                      <tr class="admin-main-account-list-infors">
                        <td class="admin-main-account-list-table-id">${myList[i].wordId}</td>
                        <td class="admin-main-account-list-table-ban">${myList[i].bannedWord}</td>
                      </tr>
                      `
            table.append(row)
        }
